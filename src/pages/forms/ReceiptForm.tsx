...
const ReceiptForm = () => {
  ...
  const [ledgerEntries, setLedgerEntries] = useState<ReceiptLedgerEntry[]>([{
    id: '1',
    ledger_id: '',
    amount: 0,
    billAllocations: []
  ...
  const [selectedEntryForBills, setSelectedEntryForBills] = useState<ReceiptLedgerEntry | null>(null);
  ...
  const loadVoucherData = async (voucherId: string) => {
    ...
    try {
      ...
      const cashDetail = details.find((detail: any) => Number(detail.debit_amount || 0) > 0);
      const partyDetails = details.filter((detail: any) => Number(detail.credit_amount || 0) > 0);

      const entries: ReceiptLedgerEntry[] = partyDetails.map((detail: any, index: number) => {
  ...
  const generateVoucherNumber = async () => {
    if (!selectedCompany) return;
    try {
      const keys = encodeURIComponent('receipt_prefix,receipt_starting_number');
      ...
      if (settingsJson && settingsJson.success) {
        ...
        const prefixSetting = settingsData.find((s: any) => s.setting_key === 'receipt_prefix');
        const numberSetting = settingsData.find((s: any) => s.setting_key === 'receipt_starting_number');
        if (prefixSetting) prefix = String(prefixSetting.setting_value || 'REC');
      ...
      if (vouchersJson && vouchersJson.success) {
        const receiptVouchers = (vouchersJson.data || []).filter((v: any) => v.voucher_type === 'receipt');
        receiptVouchers.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        if (receiptVouchers.length > 0) {
          const lastVoucher = receiptVouchers[0];
  ...
  const openBillAllocation = (entry: ReceiptLedgerEntry) => {
    if (!entry.ledger_id) {
      toast({
        title: 'Error',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.cash_bank_ledger_id) {
      toast({
        ...
        description: "Please select Receive Into account",
        variant: "destructive",
      });
      return;
    }
    ...
    if (validEntries.length === 0) {
      toast({
        title: "Validation Error",
        description: "Please add at least one receipt entry with ledger and amount",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Prepare voucher details and ledger entries payloads
      ...
      // Create ledger entries payload
      const allLedgerEntries: any[] = [];
      // Debit Cash/Bank Account (total receipt in)
      allLedgerEntries.push({
        ...
        narration: formData.narration || 'Receipt'
      });

      // Credit each party account
      ledgerEntries.filter(e => e.ledger_id && e.amount > 0).forEach(entry => {
        allLedgerEntries.push({
          ...
          isDeemedPositive: 'no',
          narration: formData.narration || `Receipt via ${ledgers.find(l => l.id === formData.cash_bank_ledger_id)?.name}`
        });
      });

      // Build full payload for backend
      const fullPayload = {
        ...voucherPayload,
        details: voucherDetails.map((detail) => {
          const entry = ledgerEntries.find((le) => le.ledger_id === detail.ledger_id && le.amount === detail.amount);
          return {
            ...detail,
        ...
        ledger_entries: allLedgerEntries.map((entry) => {
          if (entry.ledger_id === formData.cash_bank_ledger_id) {
            return entry;
          }

          const sourceEntry = ledgerEntries.find(
            (le) => le.ledger_id === entry.ledger_id && le.amount === entry.credit_amount,
          );

          return {
            ...entry,
            billallocation: (sourceEntry?.billAllocations || [])
              .filter((a) => a.allocated_amount > 0)
              .map((a) => ({
                id: a.invoice_voucher_id,
                invoice_voucher_id: a.invoice_voucher_id,
      ...
      if (!respJson || !respJson.success) throw new Error(respJson?.message || 'Failed to save voucher');
      voucher = respJson.data;

      setSavedVoucher(voucher);
      toast({
        title: "Success",
        description: editVoucherId ? "Receipt entry updated successfully!" : "Receipt entry created successfully!"
      });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to create receipt entry",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
      ...
      <html>
        ...
        <body>
          ...
          <div class="details">
            ...
            <p><strong>Received Into:</strong> ${ledgers.find(l => l.id === formData.cash_bank_ledger_id)?.name}</p>
            ${ledgerEntries.map(e => `<p><strong>Received From:</strong> ${ledgers.find(l => l.id === e.ledger_id)?.name} - ${currencySymbol} ${e.amount.toFixed(2)}</p>`).join('')}
            <p><strong>Narration:</strong> ${voucher.narration || 'Receipt Entry'}</p>
          </div>
          <div class="amount-box">
            <h3>Amount Received: ${currencySymbol} ${totalNet.toFixed(2)}</h3>
            <p>Amount in Words: ${numberToWords(totalNet)}</p>
          </div>
          <div class="signature">
            <div>Prepared By: _______________</div>
            <div>Approved By: _______________</div>
            <div>Paid By: _______________</div>
          </div>
        </body>
      </html>
    `);
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Receipt From (Ledgers)</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t">
                <div className="flex justify-between items-center">
                  <Label className="text-lg">Total Receipt Amount:</Label>
