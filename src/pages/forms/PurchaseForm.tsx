...
interface PurchaseFormProps {
  voucherType?: 'purchase' | 'debit-note';
}

const PurchaseForm = ({ voucherType = 'purchase' }: PurchaseFormProps) => {
  const actualVoucherType = voucherType || 'purchase';
  ...
  const formTitle = actualVoucherType === 'debit-note' 
    ? (isEditMode ? 'Edit Debit Note' : 'Create Debit Note')
    : (isEditMode ? 'Edit Purchase Invoice' : 'Create Purchase Invoice');
  
  const invoiceLabel = actualVoucherType === 'debit-note' ? 'Debit Note' : 'Purchase Invoice';
  ...
  // Purchase ledger selection for automatic ledger entry creation
  const [selectedPurchaseLedgerId, setSelectedPurchaseLedgerId] = useState<string>('');
  ...
  const generateBillNumber = async () => {
    ...
    try {
      ...
      const settingKeys = actualVoucherType === 'debit-note' 
        ? 'debit_note_prefix,debit_note_starting_number'
        : 'bill_prefix,bill_starting_number';
      
      // Fetch settings for bill/debit-note prefix and starting number from backend API
      const settingsResponse = await fetch(
        `http://localhost:5000/api/settings?companyId=${selectedCompany.id}&keys=${settingKeys}`
      );
      
      let prefix = actualVoucherType === 'debit-note' ? 'DN' : 'BILL';
      ...
      if (settingsResponse.ok) {
        ...
        const prefixKey = actualVoucherType === 'debit-note' ? 'debit_note_prefix' : 'bill_prefix';
        const numberKey = actualVoucherType === 'debit-note' ? 'debit_note_starting_number' : 'bill_starting_number';
        
        const prefixSetting = settingsData.find(s => s.setting_key === prefixKey);
        const numberSetting = settingsData.find(s => s.setting_key === numberKey);
        
        if (prefixSetting) prefix = String(prefixSetting.setting_value || (actualVoucherType === 'debit-note' ? 'DN' : 'BILL'));
        if (numberSetting) startingNumber = parseInt(String(numberSetting.setting_value || '1'));
      }
      
      // Get the last bill/debit-note number for this company from backend API
      ...
      if (vouchersResponse.ok) {
        ...
        // Filter for purchase/debit-note vouchers and get the last one
  ...
  const loadExistingVoucher = async (voucherId: string, preloadedLedgers: any[] = []) => {
    ...
    try {
      ...
      let loadedPurchaseItemsData: PurchaseItem[] = [];
      ...
      // Find and extract the purchase ledger (isinventory = "yes")
      const purchaseLedgerDetail =
        ...
        ledgerDetails.find((d: any) => {
          ...
          return classifier.includes('purchase');
        });
      const resolvedPurchaseLedgerId = purchaseLedgerDetail?.ledger_id || '';
      if (resolvedPurchaseLedgerId) {
        setSelectedPurchaseLedgerId(resolvedPurchaseLedgerId);
      }
      ...
      if (itemDetails && itemDetails.length > 0) {
        const purchaseItemsData = itemDetails.map((detail: any, index: number) => {
        ...
        loadedPurchaseItemsData = purchaseItemsData;
      ...
      if (ledgerDetails && ledgerDetails.length > 0) {
        ...
        const inventoryLedgerId = resolvedPurchaseLedgerId;
      ...
      if (loadedPurchaseItemsData.length > 0) {
        const recalculatedTaxLedgers = updateTaxLedgersAutomatically(
          loadedPurchaseItemsData,
          loadedAdditionalLedgersData,
        );
        calculateTotals(loadedPurchaseItemsData, recalculatedTaxLedgers);
      }
  ...
  const updatePurchaseItem = (index: number, field: keyof PurchaseItem, value: any) => {
    ...
    if (field === 'item_id' && value) {
      ...
      if (selectedItem) {
        ...
        newItems[index].rate = selectedItem.purchase_rate || 0;
    ...
    const updatedTaxLedgers = updateTaxLedgersAutomatically(newItems);
    calculateTotals(newItems, updatedTaxLedgers);
  };

  const handlePurchaseItemSearchChange = (index: number, rowId: string, inputValue: string) => {
    setItemSearchByRow((prev) => ({ ...prev, [rowId]: inputValue }));
    const match = items.find(
      (product) => product.name?.trim().toLowerCase() === inputValue.trim().toLowerCase()
    );
    if (match && purchaseItems[index]?.item_id !== match.id) {
      updatePurchaseItem(index, 'item_id', match.id);
    }
  };

  const handlePurchaseItemSearchBlur = (index: number, rowId: string) => {
    ...
    if (!entered) {
      ...
      if (purchaseItems[index]?.item_id) {
        updatePurchaseItem(index, 'item_id', '');
      }
      return;
    }

    if (exactMatch) {
      if (purchaseItems[index]?.item_id !== exactMatch.id) {
        updatePurchaseItem(index, 'item_id', exactMatch.id);
      }
      setItemSearchByRow((prev) => ({ ...prev, [rowId]: exactMatch.name }));
      return;
    }

    const selected = items.find((product) => product.id === purchaseItems[index]?.item_id);
  ...
  const openBatchSelection = (index: number) => {
    ...
    setSelectedItemForBatch({
      ...
      itemData: purchaseItems[index]?.itemData // Add itemData for tax/discount calculations
    });
  ...
  const handleBatchSelect = (batchId: string | null, allocationMethod?: 'fifo' | 'lifo', qty?: number) => {
    if (selectedItemForBatch) {
      ...
      if (qty) {
        // You could store this in a separate tracking array if needed for batch allocations
  ...
  const addAdditionalLedger = () => {
    ...
    calculateTotals(purchaseItems, newLedgers);
  };
    ...
    const newLedgers = additionalLedgers.map(entry => {
      if (entry.id === entryId) {
        return {
          ...entry,
          ledger_id: ledgerId,
          // Auto-populate tax amount for Duties & Taxes ledgers with recognized tax type
  ...
  const updateTaxLedgersAutomatically = (items: PurchaseItem[], currentLedgers?: AdditionalLedgerEntry[]) => {
    // Recalculate all tax ledger entries based on item master rates
    const updatedLedgers = (currentLedgers ?? additionalLedgers).map(entry => {
      const ledger = ledgersState.find(l => l.id === entry.ledger_id);
      const isDutiesAndTaxes = ledger?.group_name === 'Duties & Taxes';
      const hasTaxType = ledger?.tax_type && ['IGST', 'CGST', 'SGST', 'VAT'].includes(ledger.tax_type);
  ...
  useEffect(() => {
    ...
    const updatedTaxLedgers = updateTaxLedgersAutomatically(purchaseItems);
    calculateTotals(purchaseItems, updatedTaxLedgers);
  }, [purchaseItems, ledgersState]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ledger_id || formData.ledger_id === '') {
      toast({
        ...
        description: "Please select a supplier/party.",
        variant: "destructive",
      });
      return;
    }

    if (!selectedPurchaseLedgerId || selectedPurchaseLedgerId === '') {
      toast({
        title: "Validation Error",
        description: "Please select a purchase ledger.",
        variant: "destructive",
      });
      return;
    }
    
    if (purchaseItems.length === 0 || purchaseItems.every(item => !item.item_id || item.item_id === '')) {
      toast({
        title: "Validation Error", 
    ...
    try {
      ...
      // Debit Note:  Debit Supplier,   Credit Purchase/Tax (reversed = purchase return)
      const isReturn = actualVoucherType === 'debit-note';
      const ledgerEntries = [
        {
          company_id: selectedCompany.id,
          ledger_id: formData.ledger_id,
          transaction_date: formData.voucher_date,
          debit_amount: isReturn ? formData.net_amount : 0,
          credit_amount: isReturn ? 0 : formData.net_amount,
          narration: `${invoiceLabel} ${formData.voucher_number}`
        },
        {
          company_id: selectedCompany.id,
          ledger_id: selectedPurchaseLedgerId,
          transaction_date: formData.voucher_date,
          debit_amount: isReturn ? 0 : itemsSubtotal,
          credit_amount: isReturn ? itemsSubtotal : 0,
          narration: `${invoiceLabel} from ${ledgers.find(l => l.id === formData.ledger_id)?.name}`
        }
      ];
      ...
      additionalLedgers.filter(entry => entry.ledger_id && entry.amount > 0).forEach(entry => {
        ledgerEntries.push({
          ...
          transaction_date: formData.voucher_date,
          debit_amount: isReturn ? 0 : entry.amount,
          credit_amount: isReturn ? entry.amount : 0,
  ...
  const handlePrint = (voucherArg?: any) => {
    ...
    if (!voucherToPrint) {
      toast({
        title: "Error", 
        description: "Please save the purchase bill first",
        variant: "destructive"
      });
      return;
    }
    ...
    const itemsAmountTotal = purchaseItems.reduce((sum, item) => sum + Number(item.amount || 0), 0);
    const itemsTaxTotal = purchaseItems.reduce((sum, item) => sum + Number(item.tax_amount || 0), 0);
    ...
    const printableVoucherDate =
      ...
      '';
    const printableReferenceNumber =
      voucherToPrint.reference_number ||
      voucherToPrint.referenceNumber ||
      formData.reference_number ||
      '';
    const printableReferenceDate =
      voucherToPrint.reference_date ||
      voucherToPrint.referenceDate ||
    ...
    const totalQty = purchaseItems.reduce((sum, item) => sum + Number(item.quantity || 0), 0);
    const firstUnit = (() => {
      const firstItem = purchaseItems.find((item) => item.item_id);
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>${actualVoucherType === 'debit-note' ? 'Debit Note' : 'Purchase Bill'} - ${printableVoucherNumber}</title>
          <style>
        ...
        <body>
          ...
          <table class="outer">
            ...
            <tr>
              <td class="cell-pad">
                <div class="label">Supplier Invoice No. & Date.</div>
                <div>${printableReferenceNumber}${printableReferenceDate ? ` / ${printableReferenceDate}` : ''}</div>
              </td>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mt-10">
            <h2 className="text-xl font-semibold text-muted-foreground">
              Please select a company to create purchase invoices
            </h2>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{actualVoucherType === 'debit-note' ? 'Debit Note Details' : 'Purchase Details'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <SearchableDropdown
                      ...
                      options={ledgers
                        .filter((ledger) => {
                          ...
                          const targetGroups = ['Sundry Creditors', 'Sundry Debtors', 'Cash-in-Hand', 'Current Liabilities'];
                ...
                <div>
                  <Label>{actualVoucherType === 'debit-note' ? 'Note Number' : 'Bill Number'}</Label>
                  <Input 
                ...
                <div>
                  <Label>{actualVoucherType === 'debit-note' ? 'Note Date' : 'Bill Date'}</Label>
                  <Input 
                    type="date"
                    ...
                    <div>
                      ...
                      <div className="flex gap-1">
                        <Input
                          ...
                          onChange={(e) => handlePurchaseItemSearchChange(index, item.id, e.target.value)}
                          onBlur={() => handlePurchaseItemSearchBlur(index, item.id)}
                        />
                    ...
                    >
                      ...
                      <div>
                        ...
                        <Input
                          type="number"
                          value={item.rate}
                          onChange={(e) => updatePurchaseItem(index, 'rate', parseFloat(e.target.value) || 0)}
                          min="0"
                          step="0.01"
                          readOnly={isReadOnly}
                          className={isReadOnly ? 'bg-muted' : ''}
                      ...
                      {showDiscountColumn && (
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => updatePurchaseItem(index, 'discount_percent', parseFloat(e.target.value) || 0)}
                            min="0"
                            step="0.01"
                          />
                        </div>
                      ...
                      <div className="flex items-end justify-center gap-1">
                        <Button
                          ...
                          disabled={!item.item_id || !items.find(i => i.id === item.item_id)?.enable_batches}
                          title={!items.find(i => i.id === item.item_id)?.enable_batches ? "Batches disabled for this item" : (item.batch_id ? "Batch selected" : "Select batch")}
                        >
                          <Package className={`h-4 w-4 ${item.batch_id ? 'text-primary' : ''}`} />
                        </Button>
                        <Button
                          type="button"
                          variant="outline"
        ...
        <BatchSelectionDialog
          ...
          requiredQuantity={selectedItemForBatch?.quantity || 0}
        ...
        <BatchAllocationDialog
          ...
          onClose={() => setBatchAllocationDialogOpen(false)}
          itemId={selectedItemForBatch?.itemId || ''}
          itemName={selectedItemForBatch?.itemName || ''}
          requiredQuantity={selectedItemForBatch?.quantity || 0}
          companyId={selectedCompany?.id || ''}
