...
const OutstandingReceivableReport = () => {
  ...
  const fetchOutstandingData = async () => {
    ...
    try {
      ...
      const resp = await fetch(`http://localhost:5000/api/vouchers/report/outstanding-receivables?${params}`);
      if (!resp.ok) throw new Error('Failed to fetch outstanding data');
      
      const json = await resp.json();
      const receivables = json?.data || [];

      const outstanding: OutstandingItem[] = receivables
        .map((item: any) => ({
      ...
      console.log('[OutstandingReceivableReport] Fetched receivables:', {
        totalFromAPI: receivables.length,
    ...
    return (
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-muted-foreground">Please select a company to view outstanding receivables.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && outstandingData.length > 0 && (
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-center">
                  <div className="text-sm font-medium text-muted-foreground mb-2">Total Received</div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Outstanding Receivables Report</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
...
export default OutstandingReceivableReport;
